...
> **专注支持微信 4.1.9**。安装器会创建并绑定独立的 4.1.9 副本，主微信可照常使用和升级。
>
...
后续适配和回归测试集中在微信 4.1.9（build 268596 / 268602）。其他版本保留历史实现，不再承诺持续适配。

首次安装会询问是否安装 4.1.9 独立副本。确认后优先使用本地支持的 4.1.9；没有时从腾讯官方固定地址下载并校验 SHA-256。安装到 `~/Applications/WeChat-4.1.9-wechat-use.app`，独立登录、独立保存聊天数据，副本自动更新关闭。

CLI、daemon、HTTP bridge、MCP 默认使用同一副本，不需要手动加 `--bundle-id`。副本关闭时会提示打开副本，缺失或版本变化时会要求修复，不会自动连接主微信。
...
安装器先询问是否安装独立微信 4.1.9，确认后自动准备安装包和副本，并将工具绑定到它。然后打开 `~/Applications/WeChat-4.1.9-wechat-use.app` 扫码登录，再执行下面的初始化步骤。
...
- [上班摸鱼聊微信,把它搬进终端、还伪装成 dev 工具——以及背后的原理](https://blog.leeguoo.com/zh/posts/wechat-macos-moyu-terminal-disguise/) — `chat` 终端聊天(读解密 DB + 后台发送)、`disguise` 伪装 App(换 Info.plist/图标 + 重签外壳)、老板键 ⌃⌥Space(全局热键)
